...
int TestForSPITransmitDataComplete(void)    {
    ...
    return (SPI_Status & 0b10000000) >> 7;
}
...
void SPI_Init(void)
{
    ...
    SPI_Control &= 0b00100000;
    SPI_Control |= 0b01000011;
    SPI_Ext = 0x00;
    SPI_CS = 0xff;
    SPI_Status |= 0b11000000;
}
...
void SPI_ReadData(unsigned int address, int size, unsigned char *data) {
    Enable_SPI_CS();

    SPI_ReadStatusRegister();
    WriteSPIChar(0x03);
    WriteSPIChar((address & 0xFF0000) >> 16);
    WriteSPIChar((address & 0x00FF00) >> 8);
    WriteSPIChar(address & 0xFF);

    for (int i = 0; i < size; i++) {
        data[i] = WriteSPIChar(0x00);
    }
    Disable_SPI_CS();
}
...
void SPI_WriteData(int address, unsigned char* data, int size) {
...
void SPI_EraseChip(void) {
    Enable_SPI_CS();
    WriteSPIChar(0x60);
    Disable_SPI_CS();
}

void SPI_EraseSector(int address) {
    Enable_SPI_CS();
    WriteSPIChar(0x20);
    WriteSPIChar((address & 0xFF0000) >> 16);
    WriteSPIChar((address & 0x00FF00) >> 8);
...
void SPI_EraseBlock(int address) {
    Enable_SPI_CS();
    WriteSPIChar(0x52);
    WriteSPIChar((address & 0xFF0000) >> 16);
    WriteSPIChar((address & 0x00FF00) >> 8);
