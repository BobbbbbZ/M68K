; C:\M68KV6.0 - 800BY480\PROGRAMS\DEBUGMONITORCODE\M68KDEBUG.C - Compiled by CC68K  Version 5.00 (c) 1991-2005  Peter J. Fondse
...
; //     for (i = 0; i < size; i++) {
; //         MemoryAddress[i] = (unsigned char) WriteSPIChar(0x00);
; //     }
; //     Disable_SPI_CS();
; // }
; // int WriteSPIFlashData(int FlashAddress, unsigned char *MemoryAddress, int size) {
; //     int chunk_size, addr, i, base = 0;
; //     WaitForSPIFlashWriteCompletion();
; //     while (size > 0) {
; //         addr = FlashAddress + base;
...
; //         WriteSPIChar(addr >> 8);
; //         WriteSPIChar(addr);
; //         chunk_size = (size < 256)? size : 256;
; //         for (i = 0; i < chunk_size; i++) {
; //             WriteSPIChar(MemoryAddress[i + base]);
; //         }
; //         size -= chunk_size;
; //         base += chunk_size;
; //         Disable_SPI_CS();
...
; //     return 1;
; // }
; // int CheckSPIConnection (void)
; // {
; //     int status;
; //     SetSPIFlashWriteEnableLatch();
; //     status = ReadSPIFlashStatusReg();
; //     if (status == 0x02) {
; //         return 1;
; //     }
...
_ProgramFlashChip:
       rts
; //     unsigned int i, j;
; //     unsigned char *ram_ptr = ProgramStart;
; //     unsigned char Copy[Max_Size];
; //     while(!CheckSPIConnection()) {
; //         printf("\r\nError: Cannot connect to SPI Flash Chip");
; //         printf("\r\nCheck connections and press any key to try again");
; //         _getch();
; //     }
; //     printf("\r\nErasing Flash Chip ...");
; //     if (EraseSPIFlashChip() == 0) {
; //         return;
; //     }
; //     printf("\r\nProgramming Flash Chip ...");
; //     for (j = 0; j < T256K/Max_Size; j++) {
; //         if (WriteSPIFlashData(Max_Size*j, ram_ptr, Max_Size) == 0) {
; //             return;
; //         }
; //         ram_ptr += Max_Size;
; //         printf(".");
...
; //     ram_ptr = ProgramStart;
; //     for (j = 0; j < T256K/Max_Size; j++) {
; //         ReadSPIFlashData(Max_Size*j, Copy, Max_Size);
...
; //     for (j = 0; j < T256K/Max_Size; j++) {
; //         ReadSPIFlashData(Max_Size*j, Data, Max_Size);
; //         for (i = 0; i < Max_Size; i++) {
; //             ram_ptr[i] = Data[i];
; //         }
; //         ram_ptr += Max_Size;
...
; // unsigned short int *WordPtr;
; // unsigned int *LongPtr;
; // unsigned int counter1=0x400 ; // counter 1 to 0x400 to print every 1000 times
; // register unsigned int i ;
; // unsigned int Start, End, dataSize, s_start, s_end, d_start, d_end ;
; // char c ; // for user input
; // unsigned long int pattern ; // pattern to write to memory
; // start:
; // printf("\r\nMemory Test") ;
; // while(1) {
; //     printf("\r\nEnter Testing Area") ;
...
; //             Start = Get8HexDigits(0) ;
; //             if (Start < d_start || Start > d_end) {
; //                 printf("\r\nInvalid Start Address, please try again.") ; // if invalid address, print error message and continue
; //                 continue ;
; //             }
; //             printf("\r\nEnter End Address 0x08000000 - 0x0bffffff: ") ;
; //             End = Get8HexDigits(0) ;
; //             if (End < d_start || End > d_end ) {
; //                 printf("\r\nInvalid End Address, please try again.") ;
; //                 continue ;
; //             }
; //             if (Start >= End) {
...
; //     else if (c == 'L') {
; //         dataSize = 4 ;
; //         pattern = pattern | (pattern << 8)  ; // extend pattern to 32 bits
...
; // }
; // if (dataSize == 4) {
; //     LongPtr = (unsigned int *)Start ;
; // }
; // else if (dataSize == 2) {
; //     WordPtr = (unsigned short int *)Start ;
; // }
; // else {
; //     BytePtr = (unsigned char *)Start ;
; // }
; // while(1) {
; //     if (dataSize == 4) {
; //         if (LongPtr >= (unsigned int *)End) {
; //             printf("\r\nWrite Complete") ;
; //             break ;
; //         }
; //     }
; //     else if (dataSize == 2) {
; //         if (WordPtr >= (unsigned short int *)End) {
; //             printf("\r\nWrite Complete") ;
; //             break ;
; //         }
; //     }
; //     else {
; //         if (BytePtr >= (unsigned char *)End) {
; //             printf("\r\nWrite Complete") ;
; //             break ;
; //         }
; //     }
...
; char c ;
; int i, j ;
; char *BugMessage = "CPEN 412 Assignment 4a";
       lea       @m68kde~2_143.L,A0
       move.l    A0,D3
; char *CopyrightMessage = "Bob Bao 18981365";
       lea       @m68kde~2_144.L,A0
       move.l    A0,-4(A6)
; KillAllBreakPoints() ;
       jsr       _KillAllBreakPoints
...
@m68kde~2_142:
       ...
       dc.b      41,32,58,0
@m68kde~2_143:
       dc.b      67,80,69,78,32,52,49,50,32,65,115,115,105,103
       dc.b      110,109,101,110,116,32,52,97,0
@m68kde~2_144:
       dc.b      66,111,98,32,66,97,111,32,49,56,57,56,49,51
       dc.b      54,53,0
@m68kde~2_145:
